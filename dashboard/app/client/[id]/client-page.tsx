      ...
      <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
        <section>
          ...
          <p className="mt-2 text-sm font-semibold leading-snug text-slate-100">
            {event
              ? complianceView && event.masked_title
                ? event.masked_title
                : event.title
              : "Original onboarding graph before scenario replay."}
          </p>
          <p className="mt-2 text-xs leading-relaxed text-slate-500">
            {event
              ? complianceView
                ? `[GDPR MASKED MODE] Processing entity on Swiss Compliance Proxy. Token: ${report.security?.company_token || "MASKED_COMPANY_001"}`
                : event.triaged_in
          ...
          <div className="evidence-modal-panel max-h-[86vh] w-full max-w-4xl overflow-hidden rounded-lg border">
            <div className="flex items-start justify-between gap-4 border-b border-slate-800 px-6 py-5">
              <div className="min-w-0">
                ...
                <h2 className="mt-2 text-lg font-semibold leading-snug text-slate-100">
                  {complianceView && event.masked_title ? event.masked_title : event.title}
                </h2>
              </div>
              <button
                type="button"
            ...
            <div className="max-h-[62vh] overflow-y-auto px-6 py-5">
              <p className="whitespace-pre-wrap text-sm leading-7 text-slate-300">
                {complianceView
                  ? `[GDPR MASKED COMPLIANCE PREVIEW] Raw OSINT source text is loaded internally and masked locally on the Swiss Compliance Proxy prior to triggering cloud models. Token: ${report.security?.company_token || "MASKED_COMPANY_001"}`
                  : event.evidence}
              </p>
              {event.extracted_fact && (
                <div className="mt-5 rounded-lg border border-slate-800 bg-slate-900 p-4">
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-slate-300">
                    {complianceView && event.masked_title ? `Resolved fact on masked token: ${event.title.replace(report.client.legal_name, report.security?.company_token || "MASKED_COMPANY_001")}` : event.extracted_fact}
                  </p>
                </div>
              )}
            </div>
...
export default function ClientDossierPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Deterministic helper to mask names consistently
  const getMaskedName = (name: string, type: string = "company") => {
    const legalName = clientInfo?.legal_name || "";
    const companyToken = securityInfo?.company_token || "MASKED_COMPANY_001";
    const isClient = legalName
      ? name.toLowerCase().includes(legalName.toLowerCase()) ||
        legalName.toLowerCase().includes(name.toLowerCase())
      : false;
    ...
    const idx = (Math.abs(hash) % 12) + 2;
    const prefix = type.toLowerCase() === "person" ? "MASKED_PERSON" : "MASKED_COMPANY";
    return `${prefix}_${String(idx).padStart(3, "0")}`;
  };

  const displayNodes = complianceView
    ? graph.nodes.map((node) => ({
        ...node,
        label: getMaskedName(node.label, node.type),
      }))
    : graph.nodes;
    ...
    <div className="risk-workspace flex flex-col min-h-screen">
        ...
        <div className="flex items-center justify-between px-8 py-3.5">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-base font-semibold text-slate-200">
              {complianceView
                ? (report?.security?.company_token || "MASKED_COMPANY_001")
          ...
          <div className="flex items-center gap-3">
            ...
            <Separator orientation="vertical" className="h-5 bg-slate-800" />
            <div className="flex items-center gap-2 border border-slate-800 bg-slate-900/60 px-3 py-1 rounded">
              <span className="text-[11px] text-slate-400 font-semibold flex items-center gap-1">
                <ShieldCheck className={`h-3.5 w-3.5 ${complianceView ? "text-emerald-400" : "text-slate-500"}`} />
                Compliance View (GDPR Masked)
              </span>
              <button
                type="button"
                onClick={() => setComplianceView(!complianceView)}
                className={cn(
                  ...
                  complianceView ? "bg-emerald-500" : "bg-slate-700"
                )}
              >
                <span
                  className={cn(
                    ...
                    complianceView ? "translate-x-3.5" : "translate-x-0"
                  )}
                />
              </button>
            </div>
      ...
      <div className="flex-1 px-7 py-5 space-y-4">
          ...
          <Card className="col-span-3 border-slate-800 bg-slate-900 shadow-none">
            <CardHeader className="border-b border-slate-800 px-6 py-4 space-y-3">
              <CardTitle className="flex items-center gap-2.5 text-base font-semibold text-slate-200">
                ...
                {isStreaming && displayNodes.length > 0 && (
                  <span className="text-xs font-normal text-amber-400 ml-1 animate-pulse">
                    {displayNodes.length} nodes · building…
                  </span>
                )}
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  ...
                  { label: "Visible nodes", value: String(displayNodes.length), tone: "#cbd5e1" },
                  { label: "Visible links", value: String(graph.edges.length), tone: "#cbd5e1" },
                  {
                    label: "New in replay",
                    value: String(displayNodes.filter((node) => node.discoveredDuringRun).length),
                    tone: "#fbbf24",
                  },
                ].map((item) => (
            ...
            <CardContent className="p-3">
              {displayNodes.length > 0 ? (
                <CorporateGraph
                  key={`${companyId}-${scenarioStepIndex ?? "live"}-${complianceView ? "masked" : "raw"}-${displayNodes.map((n) => n.id).join("|")}`}
                  nodes={displayNodes}
                  edges={graph.edges}
                  height={620}
                />
              ...
              {isComplete && alarmFired && (
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-lg font-medium text-amber-200 leading-relaxed">
                    {complianceView && clientInfo?.legal_name
                      ? displayReport!.decision.triggering_event?.split(clientInfo.legal_name).join(securityInfo?.company_token || "MASKED_COMPANY_001")
            ...
            <CardContent className="p-6">
              <div className="flex items-start justify-between gap-6">
                <div className="flex-1 min-w-0">
                  ...
                  <h2 className="mt-1.5 text-xl font-semibold text-slate-200 leading-snug">
                    {complianceView
                      ? (securityInfo.company_token || "MASKED_COMPANY_001")
                      : clientInfo.legal_name}
                  </h2>
                  <p className="mt-1 text-sm text-slate-400 leading-relaxed max-w-xl">
                    {complianceView
                      ? clientInfo.expected_business_model.split(clientInfo.legal_name).join(securityInfo.company_token || "MASKED_COMPANY_001")
                      : clientInfo.expected_business_model}
                  </p>
                </div>
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-slate-200 truncate">
                          {complianceView ? getMaskedName(c.name, c.type) : c.name}
                        </p>
        ...
        {isComplete && !isGeneratingReport ? (
          <ComplianceReport
            markdown={displayReport!.report_markdown}
            complianceView={complianceView}
            legalName={clientInfo?.legal_name}
            companyToken={securityInfo?.company_token}
          />
        ) : (
          <Card className="border-slate-800 bg-slate-900 shadow-none">
            <CardHeader className="border-b border-slate-800 px-6 py-4">
