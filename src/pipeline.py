...
class PerpetualKYCPipeline:
    ...
    def _fallback_report(trace: dict) -> str:
        ...
        driver_line = (
            f"- Topology contagion: exposure {m['topological_exposure']} driven by "
            f"{driver['name']} ({driver['relation']}, intrinsic risk {driver['intrinsic_risk']})."
            if driver
            else "- Topology contagion: no dominant contributor."
        )
        return (
            f"# AML COMPLIANCE REPORT - ALERT RECORD {trace['alert_id']}\n\n"
            "## 1. EXECUTIVE SUMMARY\n"
            f"A statistically significant KYC drift has been detected for "
            ...
            "- Why it matters for KYC/AML: the event challenges the approved onboarding "
            "assumptions and creates an adverse signal requiring review before normal operations continue.\n"
            f"- Primary trigger: active alarm streams {m['alarms']}.\n\n"
            ...
            f"{driver_line}\n"
            f"- Transaction anomaly: active alarm streams {m['alarms']}.\n\n"
